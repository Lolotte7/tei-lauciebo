<?php
    ...
    if(!isset($_SESSION['login']))
    {
     ...
     if(isset($_GET['del']))
     {
         ...
         $delete = $bdd->prepare("DELETE FROM portfolio WHERE id=?");
         $delete->execute([$id]);
         $delete->closeCursor();
         header("LOCATION:work.php?delete=success&id=".$id);
     }
...
<div class="container">
     ...
     <div class="info-oeuvres">
        <img src="../upload/<?= $don['image'] ?>" alt="image de <?= $don['titre'] ?>">
     </div>

    <div class="info-oeuvres">
     <span class="label-oeuvres">Travaux: </span>
     <span class="value-oeuvres"><?= $don['titre'] ?></span>
    </div>

    <div class="info-oeuvres">
     <span class="label-oeuvres">Categorie: </span>
     <span class="value-oeuvres"><?= $don['categorie'] ?></span>
    </div>

    <div class="info-oeuvres">
     <span class="label-oeuvres">Description: </span><br>
     <span class="value-oeuvres"><?= nl2br($don['description']) ?></span>
    </div>

    <?php
        if(!empty($don['pdf']))
        {
            echo ' <span class="label-oeuvres">Document: </span>';
            echo "<a href='../upload/".$don['pdf']."'>Document</a>";
        }
    ?>

    <h2>Voulez-vous supprimer <?= $don['titre'] ?></h2>
    <a href="work.php" class="btn btn-success mx-2">Non</a>
    <a href="deleteWork.php?id=<?= $don['id'] ?>&del=ok" class="btn btn-danger mx-2">Oui</a>
